...
export default function Dashboard() {
  ...
  useEffect(() => {
    scan.history().then(r => setHistory(Array.isArray(r.data) ? r.data : [])).catch(() => {});
    threats.recent().then(r => setThreatList(Array.isArray(r.data) ? r.data : [])).catch(() => {});
  }, []);
  ...
  const downloadPdf = async () => {
    ...
    setDownloading(true);
    try { await reports.generate(result); } catch { alert('PDF generation failed'); }
          ...
          {alerts.slice(0, 3).map((alert, i) => (
            <div key={i} style={{
              ...
              {alert.data?.ml_prediction && (
                ...
                }}>ML: {alert.data.ml_prediction}</span>
              )}
            </div>
          ))}
        </div>
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '20px' }}>
              ...
              {stats.map((s, i) => (
                <div key={i} style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginTop: '6px' }}>{s.change}</div>
                  </div>
                </div>
              ))}
            </div>
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '0.7rem', fontWeight: 600, background: 'var(--cyan-dim)', color: 'var(--cyan)' }}>{history.length} scans</span>
              </div>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    ...
                    <Tooltip contentStyle={{ background: '#111a2d', border: '1px solid rgba(56,189,248,0.1)', borderRadius: '10px', fontSize: '0.8rem' }} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '8px' }}>
                {dist.map((d, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-3)' }}>{d.name} ({d.value})</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px' }}>
            <div style={{ marginBottom: '20px' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--text-3)', marginTop: '4px' }}>Analyze any URL for phishing, malware, and suspicious indicators</p>
            </div>
            <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
              <div style={{ flex: 1, background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: '14px', display: 'flex', alignItems: 'center', padding: '0 18px' }}>
                <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === 'Enter' && doScan()}
                  ...
                  style={{ flex: 1, padding: '16px 14px', background: 'transparent', border: 'none', color: 'var(--text-1)', fontSize: '0.92rem', outline: 'none', fontFamily: 'inherit' }}
                />
              </div>
              <button onClick={doScan} disabled={scanning} style={{
                ...
                boxShadow: '0 4px 20px rgba(56,189,248,0.25)', minWidth: '140px',
              }}>{scanning ? 'Scanning...' : 'Scan URL'}</button>
            </div>

            {result && !result.error && (
              <div style={{ marginTop: '24px', padding: '24px', borderRadius: '14px', background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '18px' }}>
                  <div>
                    <h3 style={{ fontSize: '1.25rem', fontWeight: 700 }}>{result.domain}</h3>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-3)', marginTop: '4px', fontFamily: 'JetBrains Mono, monospace' }}>{result.url}</p>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '2.2rem', fontWeight: 800, color: scoreColor(result.threat_score), lineHeight: 1 }}>{(result.threat_score * 100).toFixed(0)}%</div>
                    <span style={{ padding: '4px 14px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600, background: `${scoreColor(result.threat_score)}18`, color: scoreColor(result.threat_score) }}>{scoreLabel(result.threat_score)}</span>
                  </div>
                </div>
                <div style={{ height: 8, borderRadius: 4, background: 'var(--bg-base)', marginBottom: '22px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 4, width: `${result.threat_score * 100}%`, background: `linear-gradient(90deg, #34d399, ${scoreColor(result.threat_score)})`, transition: 'width 0.8s ease' }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px' }}>
                  {[
                    ...
                    <div key={i} style={{ padding: '18px', borderRadius: '12px', background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
                      <div style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--text-3)', marginBottom: '8px', textTransform: 'uppercase' }}>{item.l}</div>
                ...
                {result.ml_analysis?.ml_available && (
                  <div style={{ marginTop: '18px', padding: '18px', borderRadius: '12px', background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', marginBottom: '14px' }}>ML Model Analysis</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px' }}>
                      <div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginBottom: '4px' }}>Random Forest</div>
                        <div style={{ fontSize: '1.1rem', fontWeight: 800, color: result.ml_analysis.rf_prediction === 'phishing' ? '#f87171' : '#34d399' }}>{(result.ml_analysis.rf_phishing_probability * 100).toFixed(0)}%</div>
                      </div>
                      <div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginBottom: '4px' }}>Gradient Boost</div>
                        <div style={{ fontSize: '1.1rem', fontWeight: 800, color: result.ml_analysis.gb_prediction === 'phishing' ? '#f87171' : '#34d399' }}>{(result.ml_analysis.gb_phishing_probability * 100).toFixed(0)}%</div>
                      </div>
                      <div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginBottom: '4px' }}>Ensemble</div>
                        <div style={{ fontSize: '1.1rem', fontWeight: 800, color: result.ml_analysis.ensemble_prediction === 'phishing' ? '#f87171' : '#34d399' }}>{(result.ml_analysis.ensemble_score * 100).toFixed(0)}%</div>
                      </div>
                      <div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginBottom: '4px' }}>Verdict</div>
                        <span style={{ padding: '5px 12px', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 700, background: result.ml_analysis.ensemble_prediction === 'phishing' ? 'rgba(248,113,113,0.12)' : 'rgba(52,211,153,0.12)', color: result.ml_analysis.ensemble_prediction === 'phishing' ? '#f87171' : '#34d399' }}>{result.ml_analysis.ensemble_prediction}</span>
                      </div>
                    </div>
                  </div>
                )}
          ...
          {history.length > 0 && (
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                ...
                <span style={{ padding: '4px 14px', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 600, background: 'var(--cyan-dim)', color: 'var(--cyan)' }}>Last {barData.length} scans</span>
              </div>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={barData} barSize={40}>
                  <XAxis dataKey="name" tick={{ fill: '#556780', fontSize: 11 }} axisLine={{ stroke: 'rgba(56,189,248,0.06)' }} tickLine={false} />
                  <YAxis domain={[0, 1]} tick={{ fill: '#556780', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => `${(v * 100).toFixed(0)}%`} />
                  <Tooltip cursor={{ fill: 'rgba(56,189,248,0.04)' }} contentStyle={{ background: '#111a2d', border: '1px solid rgba(56,189,248,0.15)', borderRadius: '10px', fontSize: '0.82rem' }} formatter={(val) => [`${(val * 100).toFixed(0)}%`, 'Threat Score']} />
                  <Bar dataKey="score" radius={[8, 8, 0, 0]}>{barData.map((e, i) => <Cell key={i} fill={e.fill} fillOpacity={0.85} />)}</Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
            ...
            <thead>
              <tr style={{ background: 'var(--bg-surface)' }}>
                {['URL', 'Domain', 'Score', 'Status', 'Date'].map(h => (
                  <th key={h} style={{ textAlign: 'left', padding: '16px 22px', fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {history.map(s => (
                <tr key={s.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={{ padding: '16px 22px', fontSize: '0.75rem', color: 'var(--text-2)', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'JetBrains Mono, monospace' }}>{s.url}</td>
                  ...
                  <td style={{ padding: '16px 22px' }}>
                    <span style={{ padding: '5px 12px', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 600, background: s.is_malicious ? 'rgba(248,113,113,0.12)' : 'rgba(52,211,153,0.12)', color: s.is_malicious ? '#f87171' : '#34d399' }}>{s.is_malicious ? 'Malicious' : 'Safe'}</span>
                  </td>
      ...
      {tab === 'threats' && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden' }}>
          <div style={{ padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)' }}>
            ...
            <button onClick={async () => { await threats.fetchFeeds(); const r = await threats.recent(); setThreatList(Array.isArray(r.data) ? r.data : []); }}
              style={{ padding: '10px 24px', borderRadius: '10px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #38bdf8, #818cf8)', color: 'white', fontSize: '0.82rem', fontWeight: 700, fontFamily: 'inherit' }}>Refresh Feeds</button>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead>
              <tr style={{ background: 'var(--bg-surface)' }}>
            ...
            <tbody>
              {threatList.map(t => (
                <tr key={t.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={{ padding: '16px 22px', fontSize: '0.75rem', color: 'var(--text-2)', maxWidth: '420px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'JetBrains Mono, monospace' }}>{t.url}</td>
                  <td style={{ padding: '16px 22px' }}><span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 600, background: 'var(--cyan-dim)', color: 'var(--cyan)' }}>{t.source}</span></td>
                  <td style={{ padding: '16px 22px' }}><span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 600, background: 'var(--amber-dim)', color: 'var(--amber)' }}>{t.threat_type}</span></td>
