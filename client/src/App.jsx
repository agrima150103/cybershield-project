...
function Sidebar() {
  ...
  if (user?.role === 'admin') {
    links.splice(5, 0, { to: '/admin', label: 'Admin Panel', icon: '🛡️' });
  }
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-1)' }}>{user?.username}</div>
        <div style={{ fontSize: '0.75rem', color: 'var(--text-3)', marginTop: '2px' }}>{user?.email}</div>
        <button onClick={logout} style={{
          ...
          color: '#f87171', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
        }}>Logout</button>
      </div>
    </aside>
  );
